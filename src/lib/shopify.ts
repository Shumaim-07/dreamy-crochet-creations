...
const SHOPIFY_STORE_PERMANENT_DOMAIN = "dreamy-crochet-creations-knbjm-mbg4n41m.myshopify.com";
const SHOPIFY_STOREFRONT_URL = `https://${SHOPIFY_STORE_PERMANENT_DOMAIN}/api/${SHOPIFY_API_VERSION}/graphql.json`;
const SHOPIFY_STOREFRONT_TOKEN = "76413ddf4025720687dd2f3ab6e6662e";
...
export async function createShopifyCart(item: CartItem): Promise<
  { cartId: string; checkoutUrl: string; lineId: string } | null
> {
...
export async function syncShopifyCart(cartId: string): Promise<boolean> {
  try {
    ...
    if (!cart || cart.totalQuantity === 0) return false;
    return true;
  } catch (error) {
